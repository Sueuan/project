...
Dates.orien = function(obj, pos){
    ...
    obj.style.top = tops-95 + (pos ? 0 : Dates.scroll()) + 'px';
};
...
Dates.view = function(elem, options){
    ...
    as.elemv = /textarea|input/.test(Dates.elem.tagName.toLocaleLowerCase()) ? 'innerHTML' : 'innerHTML';
    if(!Dates.box){
        div = doc[creat]('div');
        div.id = as[0];
        div.className = as[0];
      ...
      //  doc.body.appendChild(div);
        $(elem).parents('.add_con').append(div);
        Dates.box = S('#'+as[0]);        
        Dates.events();
        div = null;
    } else {
    ...
    var  sTop=parseInt($('#laydate_box ').css('top'))  ;
    var scrollO=  $('.form_list ').scrollTop();
    $('.form_list ').scroll(function(){
        var thisTop=$(this).scrollTop();
        var moveTop=scrollO-thisTop;
        $('#laydate_box ').css({'top':sTop+moveTop+'px'});
    })
